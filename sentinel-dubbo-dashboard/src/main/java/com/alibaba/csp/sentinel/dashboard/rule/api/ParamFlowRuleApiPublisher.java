...
public class ParamFlowRuleApiPublisher implements DynamicRuleApiPublisher<ParamFlowRuleEntity> {
    ...
    public ParamFlowRuleApiPublisher(AppManagement appManagement, SentinelApiClient sentinelApiClient) {
    ...
    @Override
    public boolean execute(String appName, String ip, int port, List<ParamFlowRuleEntity> rules) {
        try {
            sentinelApiClient.setParamFlowRuleOfMachine(appName, ip, port, rules).get();
            return true;
        } catch (Exception ex) {
            // TODO
        }
